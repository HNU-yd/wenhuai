...
log_file <- file.path(base, "logs/10c_run_AHR_eqtlgen_MR_hybrid_match.log")
...
read_plain_dt <- function(path) {
  fread(path, sep = "\t", header = TRUE, nThread = threads, showProgress = FALSE)
}
...
prepare_exposure <- function(path) {
  ...
  required <- c(
    ...
    "eaf", "beta", "se", "pval", "samplesize", "f_stat",
...
hybrid_match_one <- function(exp_dt, out_dt) {
  n_exp <- nrow(exp_dt)

  rs_exp <- exp_dt[exp_is_rsid == TRUE]
  rs_out <- out_dt[outcome_is_rsid == TRUE]
  ...
  rs_matched_snps <- unique(rs_match$exp_SNP)

  exp_left <- exp_dt[!(exp_SNP %in% rs_matched_snps)]
  ...
  matched <- rbindlist(list(rs_match, pos_match), fill = TRUE)

  n_rsid_match <- uniqueN(rs_match$exp_SNP)
  n_pos_match <- uniqueN(pos_match$exp_SNP)
  ...
  if (nrow(matched) == 0) {
    return(list(
      ...
      qc = data.table(
        ...
        n_rsid_matched = 0,
        n_chr_pos_matched_after_rsid = 0,
        n_total_matched_unique = 0,
  ...
  allele_ok <- matched[!is.na(allele_match)]
  ...
  if (nrow(allele_ok) == 0) {
    return(list(
      ...
      qc = data.table(
        ...
        outcome_dataset = unique(out_dt$outcome_dataset),
        n_exposure_instruments = n_exp,
        n_rsid_matched = n_rsid_match,
        n_chr_pos_matched_after_rsid = n_pos_match,
        n_total_matched_unique = uniqueN(matched$exp_SNP),
        n_allele_compatible_rows = 0,
        n_dropped_ambiguous_palindromic = 0,
        n_harmonised = 0,
        missing_exposure_snps = n_exp
  ...
  if (nrow(h) > 0) {
    ...
    setorder(h, exp_SNP, match_priority, pval_outcome)
...
main <- function() {
  log_msg("Start hybrid matching MR.")
  log_msg("Rule: rsID first, then chr+pos for unmatched instruments.")
  ...
  for (inst_file in instrument_files) {
    ...
    for (out_name in names(outcomes)) {
      ...
      hx <- hybrid_match_one(exp_dt, outcomes[[out_name]])
      ...
      log_msg("rsID matched: ", qc$n_rsid_matched)
      log_msg("chr+pos matched after rsID: ", qc$n_chr_pos_matched_after_rsid)
  ...
  harmonised_out <- file.path(mr_dir, "AHR_eqtlgen_to_myocarditis_hybrid_harmonised.tsv")
  qc_out <- file.path(qc_dir, "AHR_eqtlgen_to_myocarditis_hybrid_harmonise_qc.tsv")
  mr_out <- file.path(mr_dir, "AHR_eqtlgen_to_myocarditis_hybrid_MR_results.tsv")
  single_out <- file.path(mr_dir, "AHR_eqtlgen_to_myocarditis_hybrid_singleSNP.tsv")
